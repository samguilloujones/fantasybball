...
// Mocked game date ranges (could later be pulled from Supabase)
...
export default function Scoreboard() {
    ...
    const fetchData = async () => {
      try {
        ...
        const formatted = (data || []).map((m: any) => ({
          ...
          team_a: m.team_a_id
            ? { id: m.team_a_id.id, name: m.team_a_id.name }
            : null,
          team_b: m.team_b_id
            ? { id: m.team_b_id.id, name: m.team_b_id.name }
            : null,
          winner: m.winner_id
            ? { id: m.winner_id.id, name: m.winner_id.name }
            : null,
          scores: m.scores ?? [],
        }))
  ...
  const handleNext = () => {
    setCurrentGameIndex((prev) =>
      Math.min(prev + 1, sortedKeys.length - 1)
    )
  }

  // 🗓 Format dates & determine current game
  ...
  const seasonProgress = Math.round(
    ((currentGameIndex + 1) / sortedKeys.length) * 100
  )
    ...
    <div className="p-6 max-w-5xl mx-auto space-y-6">
      ...
      <div className="space-y-5">
        {currentMatchups.map((m, idx) => {
          const scoreA =
            m.scores.find((s) => s.team_id === m.team_a?.id)?.points ?? 0
          const scoreB =
            m.scores.find((s) => s.team_id === m.team_b?.id)?.points ?? 0
