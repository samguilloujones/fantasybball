...
export default function PlayerPopup({ player, onClose }: PlayerPopupProps) {
  ...
  const loadAndCachePlayerStats = async () => {
    ...
    const seasonsToFetch = [2025, 2024, 2023, 2022, 2021];
    ...
    try {
      ...
      for (const season of seasonsToFetch) {
        ...
        if (cached) cachedSeasons.push(cached);
        else missingSeasons.push(season);
      }

      // 2️⃣ If all cached, no fetch required
      if (missingSeasons.length === 0) {
        setStats(cachedSeasons.sort((a, b) => b.season - a.season));
        setLoading(false);
        return;
      }
      ...
      for (const season of missingSeasons) {
        ...
        await fetch(`/api/supabase-proxy?path=player_stats`, {
          method: "POST",
      ...
      <div className="bg-white rounded-lg shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
        ...
        <div className="p-4 bg-white">
          <div className="grid grid-cols-2 gap-3 mb-6">
            <InfoCard label="Height" value={player.height || "N/A"} />
